...
func (c *Client) CreateBatchID(ctx context.Context) (string, error) {
	...
	if err != nil {
		return "", errors.New(fmt.Sprintf("エラーが発生しました/err: %+v", err))
	}
	// ステータスコード201以外の場合
	...
	if err := json.Unmarshal(b, &batchIDRes); err != nil {
		return "", errors.New(fmt.Sprintf("JSONのパースに失敗しました/err: %+v", err))
	}
...
func (c *Client) ValidateBatchID(ctx context.Context, batchID string) error {
	...
	case 200:
		return nil
	case 400:
		_ = logger.Log.Warnf("batchIDが無効です/batchID: %s", batchID)
		return nil
	default:
